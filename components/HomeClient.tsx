...
export function HomeClient({
  ...
  initialSharedConfig = null,
  showSeoContent = false,
}: Props) {
  const [config, setConfig] = useState<LifeConfig>(initialConfig);
  const [sharedConfig, setSharedConfig] = useState<LifeConfig | null>(
    initialSharedConfig,
  );
  const [showControls, setShowControls] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [linkCopied, setLinkCopied] = useState(false);
  const [quickMilestoneDate, setQuickMilestoneDate] = useState<Date | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      if (
        !showControls &&
        dx < -50 &&
        swipeStartX.current > window.innerWidth - 60
      ) {
    ...
    return (
      <I18nProvider value={t}>
        <div className="bg-white dark:bg-zinc-900">
          <WelcomeScreen onSubmit={(birthDate) => update({ birthDate })} />
          {showSeoContent && (
    ...
    <I18nProvider value={t}>
      <div className="min-h-screen bg-white dark:bg-zinc-900">
        ...
        <main className="mx-auto max-w-6xl px-4 py-6">
          ...
          <div
            ref={gridRef}
            className="rounded-xl border border-zinc-200 p-4 sm:p-6 dark:border-zinc-700"
          >
            <DotGrid
              birthDate={birthDate}
              lifeExpectancy={config.lifeExpectancy}
              ...
              onDotClick={
                isShared ? undefined : (date) => setQuickMilestoneDate(date)
              }
          ...
          {showSeoContent && <SeoContent locale={locale} />}
        </main>
